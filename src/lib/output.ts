...
function formatArrayTable(data: Record<string, unknown>[], columns?: Column[]): string {
	...
	const colWidths = cols.map((col) => {
		...
		const maxDataLen = data.reduce((max, row) => Math.max(max, formatValue(row[col.key]).length), 0)
		return col.width ?? Math.min(Math.max(headerLen, maxDataLen), 60)
	})

	const lines: string[] = []
	lines.push(cols.map((col, i) => `\x1b[1m${col.label.padEnd(colWidths[i]!)}\x1b[0m`).join("  "))
	lines.push(colWidths.map((w) => "─".repeat(w)).join("  "))
	for (const row of data) {
		lines.push(cols.map((col, i) => truncate(formatValue(row[col.key]), colWidths[i]!).padEnd(colWidths[i]!)).join("  "))
	}
	return lines.join("\n")
}
...
function formatObjectTable(data: Record<string, unknown>): string {
	...
	return keys.map((key) => `\x1b[1m${key.padEnd(maxKeyLen)}\x1b[0m  ${formatValue(data[key])}`).join("\n")
}
...
function formatPlain(data: unknown, columns?: Column[]): string {
	if (Array.isArray(data)) {
		if (data.length === 0) return ""
		const cols = columns ?? Object.keys((data[0] as Record<string, unknown>)!).map((key) => ({ key, label: key }))
