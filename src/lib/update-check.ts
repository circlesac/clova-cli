...
const CLI_NAME = "clova"
const NPM_PACKAGE = "clova"
const BREW_FORMULA = "circlesac/tap/clova"
...
async function readCache(): Promise<CacheEntry | null> {
	try {
		const parsed = JSON.parse(await readFile(CACHE_FILE, "utf-8")) as CacheEntry
...
function spawnRefresh(): void {
	try {
		...
		const child = spawn(process.execPath, [], { detached: true, stdio: "ignore", env: { ...rest, [REFRESH_ENV]: "1" } })
		child.unref()
	} catch {
		// best-effort
	}
...
export async function checkForUpdate(): Promise<void> {
	...
	if (cached && compareVersions(cached.latest, current) > 0) {
		printBanner(current, cached.latest, detectSource())
	}
